...
(function() {
  ...
  var TyphoonLogic = {
    ...
    confirmIssueWarning: function() {
      var checked = document.querySelectorAll('#modal-issue-warning input[type="checkbox"]:checked');
      if (checked.length === 0) { showToast('请至少选择一个下发单位', 'error'); return; }
      hideModal('modal-issue-warning');
      showToast('预警已下发至' + checked.length + '家单位', 'success');
    },
