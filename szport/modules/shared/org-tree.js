...
function _renderUnitNode(unit, activeId, mode, containerId) {
  ...
  // 判断是否有子单位（单位下挂单位）
  var childUnits = (unit.children || []).filter(function(c) { return c.type === '单位'; });
  var hasChildUnits = childUnits.length > 0;
  // 判断是否有子部门
  var childDepts = (unit.children || []).filter(function(c) { return c.type === '部门'; });
  var hasDeptChildren = (mode !== 'units-only') && childDepts.length > 0;
  // 是否有任何可展开的子节点
  var hasExpandable = hasChildUnits || hasDeptChildren;

  var html = '<div class="tree-node' + (isActive ? ' active' : '') + '" data-unit="' + unit.id + '" data-action="selectUnit" data-param="' + unit.id + '">'
    + '<span class="tree-arrow' + (hasExpandable ? (expanded.has(unit.id) ? ' open' : '') : ' empty') + '" data-action="toggleOrgTreeNode" data-param="' + unit.id + '">&#9654;</span>'
    + '<span class="node-icon unit">单</span>'
  ...
  if (hasExpandable) {
    ...
    // 先渲染子单位（递归）
    childUnits.forEach(function(sub) {
      html += _renderUnitNode(sub, activeId, mode, containerId);
    });
    // 再渲染子部门（units-only 模式下跳过）
    if (hasDeptChildren) {
      childDepts.forEach(function(dept) {
